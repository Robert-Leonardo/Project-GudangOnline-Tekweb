<?php
// File: edit_product.php (UPDATED - Multi-Gudang Logic & Tanggal Update)
error_reporting(0); 
...
if (isset($_FILES['edit_foto']) && $_FILES['edit_foto']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($fotoTmp, $path)) {
        $new_foto_path = $path; // Ganti path foto
        $file_uploaded = true;
    } else {
...
// 2. Siapkan Query UPDATE (Tambahkan tanggal_update = NOW() dan filter gudang_id)
$query = "UPDATE produk SET nama = ?, harga = ?, stok = ?, deskripsi = ?, foto = ?, tanggal_update = NOW() WHERE id = ? AND gudang_id = ?";
...
if ($stmt->execute()) {
    ...
    $stmt->close();
    echo json_encode(['status' => 'success', 'message' => 'Detail produk berhasil diupdate!']);
} else {
    // Jika gagal update DB, hapus foto baru yang terlanjur diupload
    if ($file_uploaded && $new_foto_path && file_exists($new_foto_path)) {
        unlink($new_foto_path);
    }
    $stmt->close();
